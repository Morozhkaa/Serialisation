...
import (
	...
	"time"
	"unsafe"
	"yaml-service/internal/domain/models"
...
func (s *SerializationApp) GetResult(ctx context.Context) (string, error) {
	...
	for i := 0; i < numberIterations; i++ {
		...
		data, err = student.SerializeYAML()
		if err != nil {
	...
	for i := 0; i < numberIterations; i++ {
		...
		err = models.DeserializeYAML(data, &student)
		if err != nil {
	...
	res := fmt.Sprintf("YAML - %d - %d.%dµs - %d.%dµs", unsafe.Sizeof(data), serTime/1000, serTime%1000, deserTime/1000, deserTime%1000)
	return res, err
}
