...
import (
	...
	"json-service/internal/domain/models"
...
func (s *SerializationApp) GetResult(ctx context.Context) (string, error) {
	...
	for i := 0; i < numberIterations; i++ {
		...
		data, err = student.SerializeJSON()
		if err != nil {
	...
	for i := 0; i < numberIterations; i++ {
		...
		err = models.DeserializeJSON(data, &student)
		if err != nil {
	...
	res := fmt.Sprintf("JSON - %d - %d.%dµs - %d.%dµs", unsafe.Sizeof(data), serTime/1000, serTime%1000, deserTime/1000, deserTime%1000)
	return res, err
}
