...
import (
	"context"
	"fmt"
	"gob-service/internal/domain/models"
	"time"
	"unsafe"
...
func (s *SerializationApp) GetResult(ctx context.Context) (string, error) {
	...
	for i := 0; i < numberIterations; i++ {
		...
		err = models.DeserializeGOB(data, &student)
		if err != nil {
	...
	res := fmt.Sprintf("GOB - %d - %d.%dµs - %d.%dµs", unsafe.Sizeof(data), serTime/1000, serTime%1000, deserTime/1000, deserTime%1000)
	return res, err
}
